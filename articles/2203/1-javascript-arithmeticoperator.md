...
「+」は加算演算子としての役割がありますが、実際には文字連結演算子や単項プラス演算子と呼ばれるその他の機能も持っています。
...
数値ではなく文字列に適用された場合には、お互いの文字を結合することができます。
```javascript
console.log('Hello,' + ' World!'); // Hello, World!
```

つまり、加算演算子は、数値同士、または文字列同士に演算を行います。
...
## 単項演算子
1つのオペランドを受け取る算術演算子について見ていきましょう。
...
単項マイナス演算子も、数値以外のオペランドで数値に変換できるものは、数値に変換されます。
```javascript
console.log(-'5'); // -5
console.log(-'string'); // NaN
```
...
算術演算子は、単純な足し算や引き算を行う四則演算に使われるだけでなく、値に1を増やしたり減らしたりと、さまざまな役割があります。
...
まずはキーワードだけ頭に入れ、それぞれの役割は使いながら覚えていくと良いでしょう。
